...
function Welcome() {
  ...
  useEffect(() => {
    axios.get(`${VITE_URL_EXPRESS_VERCEL}/user/${_id}`)
  ...
  useEffect(() => {
    ...
    axios.get(`${VITE_URL_EXPRESS_VERCEL}/anuncios/${_id}`)
  ...
  let eliminarAnuncio = async (idAnuncio) => {
    ...
    let peticion = await fetch (`${VITE_URL_EXPRESS_VERCEL}/delete/${idAnuncio}`, options)
    ...
    <div className='welcome__div'>
      ...
      <h2 className='welcome__h2'>{name? `${name} : esta es tu area personal de usuario!` : ''  }</h2>
        ...
        {anuncios.map((element) => {
          ...
          <button type="button" className='divCard__button' onClick={() => handleUpdateClick(element._id)}>Update</button>    
          <button type="button" className='divCard__button'onClick={() => eliminarAnuncio(element._id)} >Delete</button>
          </div>
          </div>
         )})}
